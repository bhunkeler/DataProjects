...
class Map():
    ...
    def __init__(self, config, path):
        self._config = config
        self._cities = pd.read_csv(path, sep = ',')
        self._key    = config['api_key']
        # self._client = googlemaps.Client(key = self._key)
    ...
    def city_lat_lon(self, city = 'Winterthur'):
        '''
        Retrieve coordinates lat, lon of a city
        ...
        lon        - longitude
        '''
        lat = self._cities[self._cities['city'] == city].lat.values
        lon = self._cities[self._cities['city'] == city].lon.values

        return lat[0], lon[0]

    def city_point(self, city = 'Winterthur'):
        '''
        Retrieve coordinates lat, lon of a city
    ...
    def distance_matrix(self):
        ...
        city_names = self._cities['city']
        dist_matrix = pd.DataFrame()
        dist_matrix['cities'] = city_names

        for i, target_name in enumerate(city_names):
            
            target_point = self.city_point(target_name) 
            dist = []
            
            for i, city_name in enumerate(city_names):
                
                dest_point = self.city_point(city_name)
